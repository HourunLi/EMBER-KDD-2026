...
class channel_masker(nn.Module):
    def __init__(self, args):
        ...
        self.weights = nn.Parameter(torch.distributions.Uniform(0, 1).sample((args.num_features, 2)))
...
class SAGE_encoder(nn.Module):
    def __init__(self, args, neurons_per_layer: List[int]):
        ...
        for i in range(len(neurons_per_layer) - 1):
            self.gcn_stack.append(SAGEConv(in_channels=neurons_per_layer[i], out_channels=neurons_per_layer[i + 1], aggr='mean'))
    ...
    def clip_parameters(self, channel_weights):
        for i in range(self.gcn_stack[0].lin_l.weight.data.shape[1]):
            self.gcn_stack[0].lin_l.weight.data[:, i].data.clamp_(-self.args.clip_e * channel_weights[i],
                                                        self.args.clip_e * channel_weights[i])

        for i in range(self.gcn_stack[0].lin_r.weight.data.shape[1]):
            self.gcn_stack[0].lin_r.weight.data[:, i].data.clamp_(-self.args.clip_e * channel_weights[i],
                                                        self.args.clip_e * channel_weights[i])
    ...
    def forward(self, x, edge_index, adj_norm_sp=None):       
        ...
        for i, gcn_block in enumerate(self.gcn_stack):
            x = gcn_block(x.to(torch.float), edge_index)
