...
# 自己改的
def load_syn_1(dataset,sens_attr="",predict_attr="", path="dataset/syn-1/", label_number=-1,sens_number=-1,seed=-1,test_idx=True):
    ...
    return adj_norm_sp, edge_index, features, labels, train_mask, val_mask, test_mask, sens_labels, adj
...
def get_dataset(dataname):
    ...
    adj_norm_sp, edge_index, features, labels, train_mask, val_mask, test_mask, sens, adj = load(
    ...
    elif(dataname == 'region_job' or dataname == 'pokec_n'):
        sens_idx = 3
    elif(dataname == 'syn-1'):
