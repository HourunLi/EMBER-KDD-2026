...
def load_bail(dataset, id, sens_attr="WHITE", predict_attr="RECID", path="dataset/bail/", label_number=1000):
    # print('Loading {} dataset from {}'.format(dataset, path))
    path = f"/home/disk2/lhr/fairDomainAdaption/mine/dataset/{dataset}"
    if len(dataset) > 9:
        idx_features_labels = pd.read_csv(
...
def get_dataset(dataname, inid, top_k):
    if('credit' in dataname):
        ...
        adj_norm_sp, edge_index, features, labels, train_mask, val_mask, test_mask, sens = load(
            dataset=dataname, id = inid, label_number=label_num)
    elif('bail' in dataname):
        load, label_num = load_bail, 1000
        adj_norm_sp, edge_index, features, labels, train_mask, val_mask, test_mask, sens = load(
            dataset=dataname, id = inid, label_number=label_num)
        ...
        adj_norm_sp, edge_index, features, labels, train_mask, val_mask, test_mask, sens = load(dataset=dataname, id = inid)
