    ...
    for count in pbar:
        ...
        for epoch in tqdm(range(args.epochs)):
            ...
            for epoch_c in range(0, args.cla_epochs):
                ...
                pred = (output[data.train_mask].squeeze() > 0).type_as(data.y)
                acc_train= pred.eq(data.y[data.train_mask]).sum().item() / data.train_mask.sum().item()
                auc_rocs_train = roc_auc_score(data.y[data.train_mask].cpu().numpy(), output[data.train_mask].detach().cpu().numpy())
                # print(sum(pred==1))
            ...
            if args.ood == 2:
                ...
                for i in range(len(args.strlist)):
                    # print(args.dataset, args.outid + args.strlist[i])
