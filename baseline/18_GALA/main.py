...
if __name__ == '__main__':
    args = arg_parse()
    # bailA, germanA, Pokec and syn are single-graph node-classification
    # benchmarks rather than TU graph-classification datasets. Dispatch them
    # to the fairness
    # GALA runner, while preserving the original TU entry point for the other
    # datasets shipped with this repository.
    if str(args.DS).lower() in {'baila', 'germana', 'pokec', 'syn'}:
        from bailA_gala import main as run_baila
        run_baila(args)
    else:
        # Keep the legacy TU-dataset imports lazy so the fairness runner does not
        # require the optional community-detection dependency.
        ...
        globals()['setup_seed'] = legacy_utils.setup_seed
        globals()['get_logger'] = legacy_utils.get_logger
        globals()['ce_loss'] = legacy_utils.ce_loss
