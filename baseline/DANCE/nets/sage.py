...
class GraphSAGE(nn.Module):
    def __init__(self, args):
        ...
        self.convx = nn.ModuleList([SAGEConv(nhid, nhid) for _ in range(nlayer-1)])
        self.dropout_p = dropout

        self.head = nn.Sequential(nn.Linear(nhid, nhid),
                            nn.ReLU(inplace=True),
                            nn.Linear(nhid, args.hidden))
