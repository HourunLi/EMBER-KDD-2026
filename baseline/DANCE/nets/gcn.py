...
# just for encoding without classification
class StandGCN(nn.Module):
    def __init__(self, args):
        super(StandGCN, self).__init__()
        nfeat = args.num_features
        nhid = args.hidden
        nlayer = args.n_layer
        self.convs = nn.ModuleList()
        self.bns = nn.ModuleList([nn.BatchNorm1d(nhid) for _ in range(nlayer)])
        # self.bns.append(nn.BatchNorm1d(args.feature_dim))
        for i in range(nlayer):
            if i == 0:
                self.convs.append(GCNConv(nfeat, nhid, cached= False, normalize=True))
            elif i == nlayer-1:
                self.convs.append(GCNConv(nhid, nhid, cached= False, normalize=True))
            else:
                self.convs.append(GCNConv(nhid, nhid, cached= False, normalize=True))
    ...
    def forward(self, x, adj, edge_weight=None):
        ...
        for i, conv_layer in enumerate(self.convs):
            ...
            x, edge_index = conv_layer(x, edge_index, edge_weight, is_add_self_loops=self.is_add_self_loops)
            x = self.bns[i](x)  # Apply BatchNorm for each layer
            # x = self.activate(x)
            if i > 0:
                x += learn_x[i - 1]
            learn_x.append(x)
    ...
    #         nn.init.xavier_uniform_(m.weight)  # GCNConv 内部有线性层
    ...
    #     self.initialize_weights(self.bn1)
    #     for conv, bn in zip(self.convx, self.bnx):
    #         self.initialize_weights(self.conv)
    #         self.initialize_weights(self.bn)
    def reset_parameters(self):
