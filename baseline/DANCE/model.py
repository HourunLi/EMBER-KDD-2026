...
class MLP_discriminator(torch.nn.Module):
    def __init__(self, args):
        ...
        self.lin = nn.Sequential(nn.Linear(args.feature_dim, args.hidden), self.activate, nn.Linear(args.hidden, 1), nn.Sigmoid())
...
class MLP_classifier(torch.nn.Module):
    def __init__(self, args):
        ...
        # print(self.activate)
        self.lin = nn.Sequential(nn.Linear(args.feature_dim, args.hidden), self.activate, nn.Linear(args.hidden, args.num_classes), nn.Sigmoid())
...
class MLP_projector(torch.nn.Module):
    def __init__(self, args):
        ...
        if args.activation == "ident":
            ...
            self.activate = nn.LeakyReLU()
        self.head = nn.Sequential(nn.Linear(args.feature_dim, args.hidden),
                            # nn.ReLU(),
                            nn.Linear(args.hidden, args.hidden))
    ...
    def forward(self, h):
        feat = F.normalize(self.head(h), dim=1)
        return feat
...
class MLP_encoder(torch.nn.Module):
    def __init__(self, args):
        ...
        self.lin = nn.Sequential(Linear(args.num_features, args.hidden),
                                 ...
                                 nn.Dropout(p=args.dropout),
                                 self.activate)
        # self.lin = Linear(args.num_features, args.feature_dim)
