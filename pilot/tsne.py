...
def plot_tsne(_K_classes, node_rep: np.ndarray, node_prelbl: np.ndarray, seedn, sample_num, seed):
    ...
    colors = np.array(['#4DBBD5CC','darkgrey',"limegreen",'pink',
    ...
    for bg in range(laps):
        ...
        for i in range(gap):
            axis.scatter(norm_ts_embeds[st+i, 0], norm_ts_embeds[st+i, 1], s=20, c=colors[sample_labels[st+i]], marker='o', linewidths=0.3, label = "asds")
                # c=colors[sample_labels[st+i]], marker=markers[sample_labels[st+i]], linewidths=0.3)
    # plt.xticks([])
    # plt.yticks([])
    # plt.axis('off')
    # plt.legend(loc='lower left', framealpha=0.8)
    plt.title(f"{name} t-SNE result", fontsize=15)
...
if __name__ == '__main__':
    ...
    # name = 'amap'
    name = 'FatraGNN pokec_z'
    seed = 7
    sample_num = 3000
    # seedn = 0

    node_hid = np.load('./fatra_z_feat.npz')
    node_prelabel = np.load('./fatra_z_labels.npz')
